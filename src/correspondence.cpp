...
int main(int argc, char *argv[])
{
    ...
    //  估计模型点云的分辨率 并与参数相乘Set up resolution invariance
    ...
    uniform_sampling.filter(*model_keypoints); // 降采样后的点云
    ...
    // 将关键点关联到SHOT特征描述符以执行模型aa和场景的关键点匹配并确定点对点对应关系。
    ...
    descr_est.compute(*model_descriptors); // 存储计算出的描述子
    ...
    //  Using Hough3D,需要为属于作为参数传递的云的每个关键点关联一个本地参考帧（LRF）
    if (use_hough_)
    {
    ...
    else // Using GeometricConsistency 不需要进行LRF计算
    {
